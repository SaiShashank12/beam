...
def getRemainingComments(accessToken, pr, initialComments):
  ...
  '''
    ...
    check_urls = str(
        list(map(lambda c: c["targetUrl"], commit["status"]["contexts"])))
    remainingComments = []
...
if __name__ == '__main__':
  ...
  '''
    ...
    if len(remainingComments) == 0:
        print(
            'Jobs have been started for all comments. If you would like to retry all jobs, create a new commit before running this script.'
        )
    while len(remainingComments) > 0:
        ...
        time.sleep(60)
        remainingComments = getRemainingComments(accessToken, pr,
                                                 remainingComments)
        if len(remainingComments) > 0:
            print(
                f'{len(remainingComments)} comments must be reposted because no check has been created for them: {str(remainingComments)}'
            )
            print(
                'Sleeping for 1 hour to allow Jenkins to recover and to give it time to status.'
            )
            for i in range(60):
                time.sleep(60)
        ...
        if len(remainingComments) == 0:
            print(
                f'{len(remainingComments)} comments still must be reposted: {str(remainingComments)}'
            )
