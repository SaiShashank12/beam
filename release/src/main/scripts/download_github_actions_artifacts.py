...
def parse_arguments():
  ...
  """
    ...
    parser.add_argument("--github-token-var",
                        required=False,
...
def get_last_run_id(workflow_id, repo_url, rc_tag, release_commit,
                    github_token):
    """
  Gets id of last run for given workflow, repo, branch and commit.
  Raises exception when no run found.
  """
    url = GH_API_URL_WORKFLOW_RUNS_FMT.format(repo_url=repo_url,
    ...
    print(f"GCS location corresponding to artifacts built in this run: "
...
def validate_run(run_id, repo_url, github_token):
    ...
    if status == "completed" and conclusion == "success":
        ...
        wait_for_workflow_run_to_finish(run_id, repo_url, status, conclusion,
                                        github_token)
    else:
        run_web_url = GH_WEB_URL_WORKLOW_RUN_FMT.format(repo_url=repo_url,
                                                        run_id=run_id)
        raise Exception(
...
def wait_for_workflow_run_to_finish(run_id, repo_url, status, conclusion,
                                    github_token):
    ...
    while True:
        ...
        if (now - last_request) > request_interval:
            last_request = now
            run_data = get_single_workflow_run_data(run_id, repo_url,
                                                    github_token)
            status = safe_get(run_data, "status")
            conclusion = safe_get(run_data, "conclusion")
...
def filter_artifacts(artifacts, rc_number):
    ...
    def filter_source(artifact_name):
        if rc_number:
            return artifact_name.startswith(
                "source_zip_rc{}".format(rc_number))
        return artifact_name.startswith(
            "source_zip") and "_rc" not in artifact_name

    def filter_wheels(artifact_name):
        if rc_number:
            return artifact_name.startswith(
                "wheelhouse-rc{}".format(rc_number))
        return artifact_name.startswith(
            "wheelhouse") and "-rc" not in artifact_name

    return [
        a for a in artifacts
        if (filter_source(a["name"]) or filter_wheels(a["name"]))
    ]
...
def fetch_github_artifacts(run_id, repo_url, artifacts_dir, github_token,
    ...
    if int(total_count) != len(artifacts):
        raise RuntimeError(
            f"Expected total count {total_count} different than returned list length {len(data_artifacts)}"
        )
    ...
    for artifact in filtered_artifacts:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            download_single_artifact(url, name, size_in_bytes, temp_file_path,
                                     github_token)
            extract_single_artifact(temp_file_path, artifacts_dir)


def download_single_artifact(url, name, size_in_bytes, target_file_path,
                             github_token):
...
if __name__ == "__main__":
    ...
    try:
        ...
        run_id = get_last_run_id(workflow_id, repo_url, rc_tag, release_commit,
                                 github_token)
