...
# Function to fetch URLs with retry logic
fetch_with_retry() {
    local url="$1"
    local output="$2"
    local retries=3
    ...
    for ((i=0; i<retries; i++)); do
        echo "Fetching $url (attempt $((i + 1)))..."
        if curl -fSL "$url" -o "$output"; then
            return 0
        fi
        echo "Failed to fetch $url. Retrying in $delay seconds..."
        sleep $delay
    done
    echo "Error: Unable to fetch $url after $retries attempts."
    return 1
}
