...
class LogAnalyzer():
    ...
    def __init__(self, project_id: str, gcp_bucket: str,
                 logger: logging.Logger, sinks: List[SinkCls]):
    ...
    def _create_log_sink(self, sink: SinkCls) -> None:
        ...
        sink_client = logging_client.sink(sink.name,
                                          filter_=filter_,
    ...
    def _grant_bucket_permissions(self, sink: logging_v2.Sink) -> None:
        ...
        if not writer_identity:
            self.logger.warning(
                f"Could not retrieve writer identity for sink {sink.name}. "
        ...
        if any(member in b.get("members", []) and b.get("role") == iam_role
               for b in policy.bindings):
            self.logger.debug(
                f"Sink {sink.name} already has the necessary permissions.")
            return
        ...
        self.logger.info(
            f"Granted {iam_role} to {member} on bucket {self.bucket} for sink {sink.name}."
        )
        ...
        for blob in blobs:
            ...
            for num, line in enumerate(content.splitlines(), 1):
                try:
                    ...
                    if not payload:
                        self.logger.warning(
                            f"Skipping log in blob {blob.name}, line {num}: no protoPayload found."
                        )
                        continue
                    ...
                    event_details = {
                        ...
                        payload.get("authenticationInfo",
                                    {}).get("principalEmail", "N/A"),
                        "method":
                        payload.get("methodName", "N/A"),
                        "resource":
                    ...
                    self.logger.warning(
                        f"Skipping invalid JSON log in blob {blob.name}, line {num}."
                    )
                    continue
    ...
    def send_email(self, subject: str, body: str) -> None:
        ...
        if not all([smtp_server, smtp_port_str, recipient, email, password]):
            self.logger.warning(
                "Email configuration is not fully set. Printing email instead."
            )
...
def main():
    ...
    subparsers.add_parser("initialize",
                          help="Initialize/update log sinks in GCP.")
    report_parser = subparsers.add_parser(
        "generate-report",
        help="Generate and send the weekly IAM security report.")
    report_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Do not send email, print report to console.")

    args = parser.parse_args()

    config = load_config_from_yaml(args.config)
    log_analyzer = LogAnalyzer(project_id=config["project_id"],
