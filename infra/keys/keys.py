...
def load_config() -> ConfigDict:
    ...
    if not isinstance(config['rotation_interval'],
                      int) or config['rotation_interval'] <= 0:
        raise ValueError(
            "Configuration 'rotation_interval' must be a positive integer.")
    if not isinstance(config['grace_period'],
                      int) or config['grace_period'] < 0:
        raise ValueError(
            "Configuration 'grace_period' must be a non-negative integer.")
    if 'logging_level' in config:
        if not isinstance(
                config['logging_level'], str) or config['logging_level'].strip(
                ) not in logging._nameToLevel:
...
def load_service_accounts_config() -> ServiceAccountsConfig:
    ...
    if not service_accounts_config or 'service_accounts' not in service_accounts_config:
        raise ValueError(
            "Service accounts configuration file is empty or invalid.")

    if not isinstance(service_accounts_config['service_accounts'], list):
        raise ValueError(
            "Service accounts configuration must be a list of service accounts."
        )

    for account in service_accounts_config['service_accounts']:
        if 'account_id' not in account or 'display_name' not in account:
            raise ValueError(
                "Each service account must have 'account_id' and 'display_name'."
            )
        if 'authorized_users' not in account or not isinstance(
                account['authorized_users'], list):
            raise ValueError(
                "Each service account must have a list of 'authorized_users'.")
...
class KeyService:
    ...
    def __init__(self,
                 config: ConfigDict,
                 service_accounts_config: ServiceAccountsConfig,
        ...
        if self.enable_logging:
            ...
            self.logger.setLevel(
                logging.CRITICAL + 1
            )  # Set to a level higher than CRITICAL to disable all logging
        ...
        self.service_account_manager = ServiceAccountManager(
            self.project_id, self.logger)

        if self.enable_logging:
            self.logger.info(
                f"Initialized KeyService for project: {self.project_id}")
    ...
    def _start_all_service_accounts(self) -> None:
        ...
        for account in self.service_accounts:
            ...
            authorized_users = [
                user['email'] for user in account.get('authorized_users', [])
            ]

            try:
                secret_name = f"{account_id}-key"
                # If service account and secret exists and is managed, just check permissions
                if self.service_account_manager._service_account_exists(
                        account_id
                ) and self.secret_manager_client._secret_is_managed(
                        secret_name):
                    self.logger.debug(
                        f"Service account {account_id} and secret {secret_name} already exist and are managed"
                    )
                    if self.secret_manager_client.is_different_user_access(
                            secret_name, authorized_users):
                        self.logger.debug(
                            f"Updating access policy for secret {secret_name}")
                        self.secret_manager_client.update_secret_access(
                            secret_name, authorized_users)

                # If the service account exists but the secret does not, create the secret and a key and ignore the existing keys
                elif self.service_account_manager._service_account_exists(
                        account_id
                ) and not self.secret_manager_client._secret_exists(
                        secret_name):
                    self.logger.debug(
                        f"Service account {account_id} exists but secret {secret_name} does not, creating secret and a new key"
                    )
                    self.secret_manager_client.create_secret(secret_name)
                    self.secret_manager_client.update_secret_access(
                        secret_name, authorized_users)

                    new_key = self.service_account_manager.create_service_account_key(
                        account_id)
                    new_key_id = new_key.name.split('/')[-1]
                    self.secret_manager_client.add_secret_version(
                        secret_name, new_key_id, new_key.private_key_data)

                # If neither secret nor service account exists, create and initialize both
                elif not self.service_account_manager._service_account_exists(
                        account_id
                ) and not self.secret_manager_client._secret_exists(
                        secret_name):
                    self.logger.debug(
                        f"Service account {account_id} and secret {secret_name} do not exist, creating both"
                    )
                    ...
                    secret_name = self.secret_manager_client.create_secret(
                        secret_name)
                    self.secret_manager_client.update_secret_access(
                        secret_name, authorized_users)
                    ...
                    self.logger.error(
                        f"Unexpected state for service account {account_id}")

            except Exception as e:
                self.logger.error(
                    f"Error creating service account or secret for {account_id}: {e}"
                )
    ...
    def cron(self, dry_run: bool = False) -> None:
        ...
        if dry_run:
            self.logger.info(
                "Starting cron job DRY RUN for service account key rotation")
        else:
            self.logger.info(
                "Starting cron job for service account key rotation")
        ...
        for account in self.service_accounts:
            ...
            secret_name = f"{account_id}-key"
            try:
                if self.secret_manager_client._is_key_rotation_due(
                        secret_name):
                    if dry_run:
                        self.logger.info(
                            f"[DRY RUN] Service account key for {account_id} is due for rotation, would rotate key."
                        )
                    else:
                        self.logger.info(
                            f"Service account key for {account_id} is due for rotation, rotating key"
                        )
                        new_key = self.service_account_manager.create_service_account_key(
                            account_id)
                        new_key_id = new_key.name.split('/')[-1]
                    ...
                    self.logger.debug(
                        f"Service account key for {account_id} is not due for rotation"
                    )
            except Exception as e:
                self.logger.error(
                    f"Error during cron job for service account {account_id}: {e}"
                )

        # Check for keys that have expired the grace period and delete them

        self.logger.info(
            "Checking for keys that have expired the grace period")
        ...
        for secret_id, key_ids in keys_to_delete:
            try:
                for key_id in key_ids:
                    if dry_run:
                        self.logger.info(
                            f"[DRY RUN] Would delete expired key {key_id} for secret {secret_id}"
                        )
                    else:
                        self.logger.info(
                            f"Deleting expired key {key_id} for secret {secret_id}"
                        )
                        self.service_account_manager.delete_service_account_key(
                            secret_id, key_id)
            except Exception as e:
                self.logger.error(
                    f"Error deleting expired keys for secret {secret_id}: {e}")
                continue

        if dry_run:
            self.logger.info(
                "Cron job DRY RUN for service account key rotation completed")
        else:
            self.logger.info(
                "Cron job for service account key rotation completed")
    ...
    def get_latest_service_account_key(self, account_id: str) -> str:
        ...
        """
        self.logger.info(
            f"Retrieving latest service account key for {account_id}")
        try:
            secret_name = f"{account_id}-key"
            key_bytes = self.secret_manager_client.get_latest_secret_version(
                secret_name)
            if not key_bytes:
                self.logger.warning(
                    f"No key found for service account {account_id}.")
                raise ValueError(
                    f"No key found for service account {account_id}.")

            self.logger.debug(
                f"Latest service account key for {account_id} retrieved successfully."
            )
            return key_bytes[1].decode('utf-8')
        except Exception as e:
            self.logger.error(
                f"Error retrieving latest service account key for {account_id}: {e}"
            )
            return ""
    ...
    try:
        ...
        if args.cron or args.cron_dry_run:
            ...
            if account_id not in account_ids:
                print(
                    f"Error: Service account '{account_id}' not found in configuration."
                )
            ...
            try:
                ...
                print(
                    f"Permission denied when accessing the key for {account_id}: {e}"
                )
                sys.exit(1)
            except Exception as e:
            ...
            print(
                "You must specify either --cron to run the cron job or --get-key <ACCOUNT_ID> to retrieve a key."
            )
