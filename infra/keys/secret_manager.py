...
class SecretManager:
    ...
    logger: Union[logging.Logger,
                  logging.LoggerAdapter]  # Logger for logging messages
    ...
    def __init__(self,
        ...
        self.logger.info(
            f"Initialized SecretManager for project '{self.project_id}'")
    ...
    def _get_secret_ids(self) -> List[str]:
        ...
        """
        self.logger.debug(
            f"Retrieving secrets with the label from project '{self.project_id}'"
        )
        secret_ids = []
        ...
        try:
            for secret in self.client.list_secrets(
                ...
                if "created_by" in secret.labels and secret.labels[
        ...
        self.logger.debug(
            f"Found {len(secret_ids)} secrets created by {SECRET_MANAGER_LABEL} in project '{self.project_id}'"
        )
        return secret_ids
    ...
    def _secret_is_managed(self, secret_id: str) -> bool:
        ...
        """
        self.logger.debug(
            f"Checking if secret '{secret_id}' exists and is managed by {SECRET_MANAGER_LABEL}"
        )
        if not self._secret_exists(secret_id):
            self.logger.debug(
                f"Secret '{secret_id}' does not exist, cannot be managed")
            return False

        name = self.client.secret_path(self.project_id, secret_id)
        secret = self.client.get_secret(request={"name": name})

        is_managed = "created_by" in secret.labels and secret.labels[
            "created_by"] == SECRET_MANAGER_LABEL
        self.logger.debug(
            f"Secret '{secret_id}' is managed by {SECRET_MANAGER_LABEL}: {is_managed}"
        )
        return is_managed
    ...
    def create_secret(self, secret_id: str) -> str:
        ...
        if self._secret_is_managed(secret_id):
            self.logger.debug(
                f"Secret '{secret_id}' already exists, returning existing secret path"
            )
            name = self.client.secret_path(self.project_id, secret_id)
            return name

        self.logger.info(
            f"Creating new secret '{secret_id}' with rotation interval of {self.rotation_interval} days"
        )
        ...
        for _ in range(self.max_retries):
            if self._secret_is_managed(secret_id):
                ...
                break
            self.logger.debug(
                f"Secret '{secret_id}' not found, retrying in {delay} seconds")
            time.sleep(delay)
            delay *= 2
        else:
    ...
    def delete_secret(self, secret_id: str) -> None:
        ...
        if not self._secret_is_managed(secret_id):
            self.logger.debug(
                f"Secret '{secret_id}' is not managed by this service, cannot delete"
            )
            return
        ...
        for _ in range(self.max_retries):
            if not self._secret_exists(secret_id):
                ...
                break
            self.logger.debug(
                f"Secret '{secret_id}' still exists, retrying in {delay} seconds"
            )
            time.sleep(delay)
            delay *= 2
        else:
    ...
    def is_different_user_access(self, secret_id: str,
        ...
        """
        self.logger.debug(
            f"Checking if access for secret '{secret_id}' differs from allowed users: {allowed_users}"
        )

        if not self._secret_is_managed(secret_id):
            self.logger.debug(
                f"Secret '{secret_id}' is not managed by this service, cannot check access"
            )
            return True
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get IAM policy for secret '{secret_id}': {e}")
            return True
        ...
        self.logger.debug(
            f"Access for secret '{secret_id}' differs: {is_different}")
        return is_different

    def update_secret_access(self, secret_id: str,
        ...
        """
        self.logger.debug(
            f"Updating access for secret '{secret_id}' to allow users: {allowed_users}"
        )
        ...
        for binding in policy.bindings:
            if binding.role == accessor_role:
                ...
                self.logger.debug(
                    f"Replaced members for role '{accessor_role}' in secret '{secret_id}' with: {allowed_users}"
                )
                binding_found = True
                break
        ...
        if not binding_found:
            ...
            self.logger.debug(
                f"Created new binding for role '{accessor_role}' in secret '{secret_id}'"
            )

        self.client.set_iam_policy(request={
            "resource": resource_name,
            "policy": policy
        })

        self.logger.info(
            f"Successfully updated access for secret '{secret_id}' to allow users: {allowed_users}"
        )

    def _get_secret_versions(
            self, secret_id: str) -> List[secretmanager.SecretVersion]:
        """
       Retrieves all versions of a secret.
        ...
        if not self._secret_is_managed(secret_id):
            self.logger.debug(
                f"Secret '{secret_id}' is not managed by this service, cannot retrieve versions"
            )
            return []

        parent = self.client.secret_path(self.project_id, secret_id)
        versions = list(
            self.client.list_secret_versions(request={"parent": parent}))
        self.logger.debug(
            f"Found {len(versions)} versions for secret '{secret_id}'")
        return versions
    ...
    def _secret_version_exists(self, secret_id: str, version_id: str) -> bool:
        ...
        """
        self.logger.debug(
            f"Checking if version '{version_id}' exists for secret '{secret_id}'"
        )
        if not self._secret_is_managed(secret_id):
            self.logger.debug(
                f"Secret '{secret_id}' is not managed by this service, cannot check version existence"
            )
            return False

        versions = self._get_secret_versions(secret_id)
        exists = any(
            version.name.split("/")[-1] == version_id for version in versions)
        self.logger.debug(f"Version '{version_id}' exists: {exists}")
        return exists

    def _secret_version_is_enabled(self, secret_id: str,
                                   version_id: str) -> bool:
        """
        ...
        """
        self.logger.debug(
            f"Checking if version '{version_id}' of secret '{secret_id}' is enabled"
        )
        if not self._secret_is_managed(secret_id):
            self.logger.debug(
                f"Secret '{secret_id}' is not managed by this service, version cannot be enabled"
            )
            return False
        ...
        for version in versions:
            if version.name.split("/")[-1] == version_id:
                ...
                self.logger.debug(
                    f"Version '{version_id}' is enabled: {is_enabled}")
                return is_enabled
        self.logger.debug(
            f"Version '{version_id}' does not exist for secret '{secret_id}'")
        return False

    def _secret_version_is_destroyed(self, secret_id: str,
                                     version_id: str) -> bool:
        """
        ...
        """
        self.logger.debug(
            f"Checking if version '{version_id}' of secret '{secret_id}' is destroyed"
        )
        if not self._secret_is_managed(secret_id):
            self.logger.debug(
                f"Secret '{secret_id}' is not managed by this service, version cannot be destroyed"
            )
            return False
        ...
        for version in versions:
            if version.name.split("/")[-1] == version_id:
                ...
                self.logger.debug(
                    f"Version '{version_id}' is destroyed: {is_destroyed}")
                return is_destroyed
        self.logger.debug(
            f"Version '{version_id}' does not exist for secret '{secret_id}'")
        return False
    ...
    def _get_latest_secret_version_id(self, secret_id: str) -> str:
        ...
        """
        self.logger.debug(
            f"Retrieving latest enabled version of secret '{secret_id}'")
        if not self._secret_is_managed(secret_id):
        ...
        for version in self._get_secret_versions(secret_id):
            if version.state == secretmanager.SecretVersion.State.ENABLED:
                ...
                self.logger.debug(
                    f"Found latest enabled version '{version_id}' for secret '{secret_id}'"
                )
                return version_id
    ...
    def _is_key_rotation_due(self, secret_id: str) -> bool:
        ...
        """
        self.logger.debug(
            f"Checking if key rotation is due for secret '{secret_id}'")
        if not self._secret_is_managed(secret_id):
        ...
        self.logger.debug(
            f"Key rotation due for secret '{secret_id}': {is_due}")
        return is_due

    def add_secret_version(self, secret_id: str, data_id: str,
                           payload: Union[bytes, str]) -> str:
        """
        ...
        if isinstance(payload, str):
            ...
            payload = f"{data_id}:{payload.decode('utf-8')}" if isinstance(
        ...
        self.logger.debug(
            f"Updating last version created timestamp for secret '{secret_id}'"
        )
        secret_obj = self.get_secret(secret_id)
        labels = dict(secret_obj.labels)
        labels["last_version_created_at"] = datetime.now(
        ...
        self.logger.debug(
            f"Waiting for new version '{version_id}' of secret '{secret_id}' to be available"
        )
        delay = 1
        for _ in range(self.max_retries):
            if self._secret_version_exists(secret_id, version_id):
                self.logger.debug(
                    f"Version '{version_id}' of secret '{secret_id}' is now available"
                )
                break
            self.logger.debug(
                f"Version '{version_id}' of secret '{secret_id}' not found, retrying in {delay} seconds"
            )
            time.sleep(delay)
            delay *= 2
        else:
        ...
        for ver in self._get_secret_versions(secret_id):
            if ver.name != response.name and ver.state == secretmanager.SecretVersion.State.ENABLED:
                self.logger.debug(
                    f"Disabling previous version '{ver.name}' of secret '{secret_id}'"
                )
                self.disable_secret_version(secret_id, ver.name.split("/")[-1])

        self.logger.info(
            f"Successfully added version '{version_id}' to secret '{secret_id}'"
        )
        return response.name
    ...
    def get_latest_secret_version(self, secret_id: str) -> Tuple[str, bytes]:
        ...
        self.logger.debug(
            f"Accessing secret version '{version_id}' of secret '{secret_id}'")
        ...
        if int(crc32c.hexdigest(), 16) != response.payload.data_crc32c:
            ...
            self.logger.error(
                f"{error_msg} for secret '{secret_id}' version '{version_id}'")
            raise ValueError(error_msg)

        self.logger.info(
            f"Successfully retrieved version '{version_id}' of secret '{secret_id}'"
        )
    ...
    def enable_secret_version(self, secret_id: str, version_id: str) -> None:
        ...
        """
        self.logger.info(
            f"Enabling version '{version_id}' of secret '{secret_id}'")
        ...
        self.logger.debug(
            f"Verifying version '{version_id}' exists for secret '{secret_id}'"
        )
        version_exists = any(
            version.name.split("/")[-1] == version_id
            and version.state == secretmanager.SecretVersion.State.DISABLED
            for version in self._get_secret_versions(secret_id))
        ...
        if response.name.split(
                "/"
        )[-1] != version_id or response.state != secretmanager.SecretVersion.State.ENABLED:
        ...
        self.logger.debug(
            f"Waiting for version '{version_id}' of secret '{secret_id}' to be enabled"
        )
        delay = 1
        for _ in range(self.max_retries):
            if self._secret_version_is_enabled(secret_id, version_id):
                self.logger.debug(
                    f"Version '{version_id}' of secret '{secret_id}' is now enabled"
                )
                break
            self.logger.debug(
                f"Version '{version_id}' of secret '{secret_id}' still disabled, retrying in {delay} seconds"
            )
            time.sleep(delay)
            delay *= 2
        else:
        ...
        self.logger.info(
            f"Successfully enabled version '{version_id}' of secret '{secret_id}'"
        )
    ...
    def disable_secret_version(self, secret_id: str, version_id: str) -> None:
        ...
        """
        self.logger.info(
            f"Disabling version '{version_id}' of secret '{secret_id}'")
        ...
        if response.name.split(
                "/"
        )[-1] != version_id or response.state != secretmanager.SecretVersion.State.DISABLED:
        ...
        self.logger.debug(
            f"Waiting for version '{version_id}' of secret '{secret_id}' to be disabled"
        )
        delay = 1
        for _ in range(self.max_retries):
            if not self._secret_version_is_enabled(secret_id, version_id):
                self.logger.debug(
                    f"Version '{version_id}' of secret '{secret_id}' is now disabled"
                )
                break
            self.logger.debug(
                f"Version '{version_id}' of secret '{secret_id}' still enabled, retrying in {delay} seconds"
            )
            time.sleep(delay)
            delay *= 2
        else:
        ...
        self.logger.info(
            f"Successfully disabled version '{version_id}' of secret '{secret_id}'"
        )
    ...
    def destroy_secret_version(self, secret_id: str, version_id: str) -> str:
        ...
        """
        self.logger.info(
            f"Destroying version '{version_id}' of secret '{secret_id}'")
        ...
        if not self._secret_version_is_enabled(secret_id, version_id):
            self.logger.debug(
                f"Version '{version_id}' of secret '{secret_id}' is not enabled, enabling it before destruction"
            )
        ...
        self.logger.debug(
            f"Data ID for version '{version_id}' of secret '{secret_id}': {data_id}"
        )
        ...
        if response.name.split(
                "/"
        )[-1] != version_id or response.state != secretmanager.SecretVersion.State.DESTROYED:
        ...
        self.logger.debug(
            f"Waiting for version '{version_id}' of secret '{secret_id}' to be destroyed"
        )
        delay = 1
        for _ in range(self.max_retries):
            if self._secret_version_is_destroyed(secret_id, version_id):
                self.logger.debug(
                    f"Version '{version_id}' of secret '{secret_id}' is now destroyed"
                )
                break
            self.logger.debug(
                f"Version '{version_id}' of secret '{secret_id}' still not destroyed, retrying in {delay} seconds"
            )
            time.sleep(delay)
            delay *= 2
        else:
        ...
        self.logger.info(
            f"Successfully destroyed version '{version_id}' of secret '{secret_id}'"
        )
        return data_id
        ...
        for version in self._get_secret_versions(secret_id):
            if version.state == secretmanager.SecretVersion.State.DISABLED:
                ...
                create_time = datetime.fromtimestamp(
                    version.create_time.timestamp(),
                    tz=timezone.utc)  # type: ignore
                if create_time < datetime.now(
                        timezone.utc) - timedelta(days=self.grace_period):
                    self.logger.debug(
                        f"Destroying disabled version '{version_id}' of secret '{secret_id}'"
                    )
                    data_ids.append(
                        self.destroy_secret_version(secret_id, version_id))
                else:
                    self.logger.debug(
                        f"Skipping version '{version_id}' of secret '{secret_id}' as it is within the grace period"
                    )
    ...
    def cron(self) -> Dict[str, List[str]]:
        ...
        for secret_id in self._get_secret_ids():
            self.logger.debug(
                f"Processing secret '{secret_id}' for maintenance")
            purged_data_ids = self.purge_disabled_secret_versions(secret_id)
            if purged_data_ids:
                self.logger.info(
                    f"Purged disabled versions of secret '{secret_id}': {purged_data_ids}"
                )
