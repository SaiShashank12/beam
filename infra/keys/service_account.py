...
class ServiceAccountManager:
    ...
    def __init__(self,
        ...
        self.logger.info(
            f"Initialized ServiceAccountManager for project: {self.project_id}"
        )
    ...
    def _normalize_account_email(self, account_id: str) -> str:
        ...
        if "@" in account_id and account_id.endswith(
    ...
    def _get_service_accounts(self) -> List[iam_admin_v1.ServiceAccount]:
        ...
        self.logger.debug(
            f"Listed service accounts: {[account.email for account in accounts.accounts]}"
        )
    ...
    def create_service_account(
            self,
            account_id: str,
            display_name: Optional[str] = None) -> types.ServiceAccount:
        """
        ...
        try:
            ...
            for _ in range(self.max_retries):
                ...
                self.logger.error(
                    f"Service account {account_id} creation timed out after {self.max_retries} retries."
                )
                raise exceptions.DeadlineExceeded(
                    f"Service account {account_id} creation timed out.")
            ...
            self.logger.info(
                f"Service account already exists: {existing_account.email}")
    ...
    def get_service_account(self, account_id: str) -> types.ServiceAccount:
        ...
        try:
            ...
            self.logger.info(
                f"Retrieved service account: {service_account.email}")
    ...
    def enable_service_account(self, account_id: str) -> None:
        ...
        for _ in range(self.max_retries):
            ...
            self.logger.error(
                f"Service account {account_id} enabling timed out after {self.max_retries} retries."
            )
            raise exceptions.DeadlineExceeded(
                f"Service account {account_id} enabling timed out.")
    ...
    def disable_service_account(self, account_id: str) -> None:
        ...
        for _ in range(self.max_retries):
            ...
            self.logger.error(
                f"Service account {account_id} disabling timed out after {self.max_retries} retries."
            )
            raise exceptions.DeadlineExceeded(
                f"Service account {account_id} disabling timed out.")
    ...
    def delete_service_account(self, account_id: str) -> None:
        ...
        for _ in range(self.max_retries):
            ...
            self.logger.error(
                f"Service account {account_id} deletion timed out after {self.max_retries} retries."
            )
            raise exceptions.DeadlineExceeded(
                f"Service account {account_id} deletion timed out.")

        self.logger.info(f"Deleted service account: {account_id}")

    def _get_service_account_keys(
            self, account_id: str) -> List[iam_admin_v1.ServiceAccountKey]:
        """
    ...
    def _service_account_key_exists(self, account_id: str,
                                    key_id: str) -> bool:
        """
    ...
    def create_service_account_key(self,
                                   account_id: str) -> types.ServiceAccountKey:
        """
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                service_account = self.client.get_service_account(
                    request=get_request)
                if service_account.disabled:
                    self.logger.info(
                        f"Service account {account_id} is disabled. Enabling it first."
                    )
                ...
                key_delay = 1
                for _ in range(self.max_retries):
                    if self._service_account_key_exists(
                            account_id,
                            key.name.split('/')[-1]):
                        break
                    ...
                    self.logger.error(
                        f"Service account key creation for {account_id} timed out after {self.max_retries} retries."
                    )
                    raise exceptions.DeadlineExceeded(
                        f"Service account key creation for {account_id} timed out."
                    )

                self.logger.info(
                    f"Created service account key for {account_id}")
                return key

            except exceptions.NotFound as e:
                if attempt < self.max_retries - 1:
                    self.logger.warning(
                        f"Service account {account_id} not found (attempt {attempt + 1}/{self.max_retries}), retrying in {delay}s. This may be due to propagation delay."
                    )
                    time.sleep(delay)
                    delay *= 2
                else:
                    self.logger.error(
                        f"Service account {account_id} not found after {self.max_retries} attempts"
                    )
                    raise
            except Exception as e:
                # For other exceptions, don't retry
                self.logger.error(
                    f"Error creating service account key for {account_id}: {e}"
                )
                raise
    ...
    def delete_service_account_key(self, account_id: str, key_id: str) -> None:
        ...
        try:
            ...
            self.logger.warning(
                f"Service account key {key_id} not found for account: {account_id} (may have been already deleted)"
            )
            raise
        except exceptions.FailedPrecondition as e:
            self.logger.warning(
                f"Failed to delete service account key {key_id} for account: {account_id}. Error: {e}"
            )
            raise
        except Exception as e:
            self.logger.error(
                f"Unexpected error deleting service account key {key_id} for account: {account_id}. Error: {e}"
            )
            raise
        ...
        for _ in range(self.max_retries):
            ...
            self.logger.error(
                f"Service account key deletion for {account_id} timed out after {self.max_retries} retries."
            )
            raise exceptions.DeadlineExceeded(
                f"Service account key deletion for {account_id} timed out.")

        self.logger.info(
            f"Deleted service account key: {key_id} for account: {account_id}")
    ...
    def test_service_account_key(self, key_data: bytes) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Invalid JSON in service account key: {json_error}")
            return False
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                self.logger.info(
                    f"Service account key is valid and can authenticate")
                return True
                ...
                if attempt < self.max_retries - 1:  # Don't log on the last attempt
                    delay *= 2
                    self.logger.warning(
                        f"Authentication attempt {attempt + 1} failed (will retry in {delay}s): {auth_error}"
                    )
                    time.sleep(delay)
                else:
                    self.logger.error(
                        f"Authentication failed with service account key after {self.max_retries} attempts: {auth_error}"
                    )
                    return False
