...
class AccountKeysPolicyComplianceCheck:
    ...
    def _get_all_live_service_accounts(self) -> List[str]:
        ...
        try:
            accounts = self.service_account_client.list_service_accounts(
                request=request)
            self.logger.debug(
                f"Retrieved {len(accounts.accounts)} service accounts for project {self.project_id}"
            )

            if not accounts:
                self.logger.warning(
                    f"No service accounts found in project {self.project_id}.")
                return []

            return [
                self._normalize_account_email(account.email)
                for account in accounts.accounts if not account.disabled
            ]
        except Exception as e:
            self.logger.error(
                f"Failed to retrieve service accounts for project {self.project_id}: {e}"
            )
            raise
    ...
    def _get_all_live_managed_secrets(self) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(
                f"Retrieved {len(secrets)} secrets for project {self.project_id}"
            )

            if not secrets:
                self.logger.warning(
                    f"No secrets found in project {self.project_id}.")
                return []

            return [
                secret.name.split("/")[-1] for secret in secrets
                if "created_by" in secret.labels
                and secret.labels["created_by"] == SECRET_MANAGER_LABEL
            ]
        except Exception as e:
            self.logger.error(
                f"Failed to retrieve secrets for project {self.project_id}: {e}"
            )
            raise
    ...
    def _get_all_secret_authorized_users(self, secret_id: str) -> List[str]:
        ...
        try:
            policy = self.secret_client.get_iam_policy(
                request={"resource": resource_name})
            self.logger.debug(
                f"Retrieved IAM policy for secret '{secret_id}': {policy}")

            if not policy.bindings:
                self.logger.warning(
                    f"No IAM bindings found for secret '{secret_id}'.")
                return []
    ...
    def _read_service_account_keys(self) -> ServiceAccountsConfig:
        ...
        try:
            ...
            self.logger.info(
                f"Service account keys file {self.service_account_keys_file} not found, starting with empty configuration"
            )
            return {"service_accounts": []}
        except IOError as e:
...
# limitations under the License.
        ...
        try:
            with open(output_file, "w") as file:
                ...
                yaml.dump(yaml_data,
                          ...
                          indent=2)
            self.logger.info(
                f"Successfully wrote Service Account Keys policy data to {output_file}"
            )
        except IOError as e:
    ...
    def check_compliance(self) -> List[str]:
        ...
        if not file_service_accounts:
            ...
            self.logger.info(
                f"No service account keys found in the {self.service_account_keys_file}."
            )
        ...
        for account in file_service_accounts:
            ...
            authorized_users = [
                user["email"] for user in account["authorized_users"]
            ]
            actual_users = [
                self._denormalize_username(user)
                for user in self._get_all_secret_authorized_users(secret_name)
            ]
    ...
    def generate_compliance(self) -> None:
        ...
        service_account_data = self._read_service_account_keys()
        file_service_accounts = service_account_data.get(
            "service_accounts", [])
        ...
        self.logger.info(
            f"Found {len(file_service_accounts)} existing service accounts in the keys file"
        )
        ...
        for service_account in self._get_all_live_service_accounts():
            if self._denormalize_account_email(service_account) not in [
                ...
                self.logger.info(
                    f"Service account '{service_account}' is not declared in the service account keys file, adding it"
                )
        ...
        for secret in managed_secrets:
            if secret not in extracted_secrets:
                self.logger.info(
                    f"Managed secret '{secret}' is not declared in the service account keys file, adding it"
                )
                file_service_accounts.append({
                    "account_id":
                    secret.strip("-key"),
        ...
        for account in file_service_accounts:
            ...
            if not authorized_users:
                self.logger.info(
                    f"Managed secret '{account}' is new, skipping permission check"
                )
                continue

            actual_users_normalized = sorted(
                self._get_all_secret_authorized_users(secret_name))
            actual_users = sorted([
                self._denormalize_username(user)
                for user in actual_users_normalized
            ])

            if authorized_users != actual_users:
                self.logger.info(
                    f"Managed secret '{account}' does not have the correct permissions, updating it"
                )
        ...
        for account in file_service_accounts:
            if account["account_id"] not in seen_accounts:
                ...
                self.logger.info(
                    f"Removing duplicate entry for account '{account['account_id']}'"
                )

        self._to_yaml_file(deduplicated_accounts,
                           self.service_account_keys_file,
                           header_info="Service Account Keys")
...
def config_process() -> Dict[str, str]:
    ...
    config_res["service_account_keys_file"] = config.get(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Account Keys Compliance Checker")
    parser.add_argument(
        "--action",
    ...
    logger.info(
        f"Starting Account Keys policy compliance check with action: {action}")
    account_keys_checker = AccountKeysPolicyComplianceCheck(
        config["project_id"], config["service_account_keys_file"], logger,
        sending_client)
    ...
    try:
        if action == "check":
            ...
            logger.info(
                "Generating new compliance based on current Account Keys policy..."
            )
