    ...
    def parse_query_to_regex(query):
        ...
        if match:
            ...
            if re.match(r"(\w+.\.)(.*)", without_quotes):
                without_retention_policy = re.sub(r"(\w+.)(.*)", r"\2",
                                                  without_quotes)

            replaced_parameters = re.sub(r"\$\{\w+\}", r"[\\w\\d]*",
...
def guess_dashboard_by_measurement(measurement,
                                   directory,
                                   additional_query_substrings=None):
    """
