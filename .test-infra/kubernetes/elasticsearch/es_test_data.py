...
def delete_index(idx_name):
    try:
        ...
        logging.info('Deleting index  "%s" done   %s' %
                     (idx_name, response.body))
...
def create_index(idx_name):
    ...
    try:
        ...
        request = tornado.httpclient.HTTPRequest(
            ...
            auth_password=tornado.options.options.password)
        response = tornado.httpclient.HTTPClient().fetch(request)
        logging.info('Creating index "%s" done   %s' %
                     (idx_name, response.body))
    except tornado.httpclient.HTTPError:
...
def upload_batch(upload_data_txt):
    ...
    logging.info("Upload: %s - upload took: %5dms, total docs uploaded: %7d" %
...
def get_data_for_format(format, count):
    ...
    if field_type == "bool":
        ...
        mydict = dict(a=field_name + str(count),
                      b=field_name + str(count),
                      c=field_name + str(count),
                      d=field_name + str(count),
                      e=field_name + str(count),
                      f=field_name + str(count),
                      g=field_name + str(count),
                      h=field_name + str(count),
                      i=field_name + str(count),
...
def set_index_refresh(val):
    ...
    url = "%s/%s/_settings" % (tornado.options.options.es_url,
...
def generate_test_data():
    ...
    if tornado.options.options.dict_file:
        ...
        logging.info("Loaded %d words from the %s" %
...
if __name__ == '__main__':
    tornado.options.define("es_url",
                           type=str,
                           default='http://localhost:9200/',
                           help="URL of your Elasticsearch node")
    tornado.options.define("index_name",
                           type=str,
                           default='test_data',
                           help="Name of the index to store your messages")
    tornado.options.define("index_type",
                           type=str,
                           default='test_type',
                           help="Type")
    tornado.options.define("batch_size",
                           type=int,
                           default=1000,
                           help="Elasticsearch bulk index batch size")
    tornado.options.define("num_of_shards",
                           type=int,
                           default=2,
                           help="Number of shards for ES index")
    tornado.options.define("http_upload_timeout",
                           type=int,
                           default=3,
                           help="Timeout in seconds when uploading data")
    tornado.options.define("count",
                           type=int,
                           default=100000,
                           help="Number of docs to generate")
    tornado.options.define("format",
                           type=str,
                           default='name:str,age:int,last_updated:ts',
                           help="message format")
    tornado.options.define("num_of_replicas",
                           type=int,
                           default=0,
    ...
    tornado.options.define("password",
                           type=str,
                           default=None,
                           help="Password for elasticsearch")
