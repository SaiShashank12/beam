...
def fetchJobs():
    ...
    jobs = r.json()['jobs']
    result = map(
        lambda x: (x['name'], int(x['lastCompletedBuild']['id'])
                   if x['lastCompletedBuild'] is not None else -1, x['url']),
        jobs)
    return result
    ...
    while not conn:
        try:
            conn = psycopg2.connect(
                f"dbname='{dbname}' user='{dbusername}' host='{host}'"
...
def fetchBuildsForJob(jobUrl):
    durFields = (
        'blockedDurationMillis,buildableDurationMillis,'
...
def buildRowValuesArray(jobName, build):
    timings = next((x for x in build['actions'] if ('_class' in x) and (
        x['_class'] == 'jenkins.metrics.impl.TimeInQueueAction')), None)
    values = [
        jobName,
        int(build['id']), build['url'], build['result'],
        datetime.fromtimestamp(build['timestamp'] / 1000), build['builtOn'],
        build['duration'], build['estimatedDuration'],
...
def insertRow(cursor, rowValues):
    cursor.execute(
        f'insert into {jenkinsBuildsTableName} values (%s, %s, %s, %s,'
