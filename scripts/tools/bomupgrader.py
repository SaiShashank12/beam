...
"""
    ...
    def prepare_gradle(self, bom_version):
        ...
        deps = []
        for dep in list(
                self.KNOWN_DEPS.values()) + self.OTHER_CONSTRANTS + list(
        ...
        with open(os.path.join(self.BUILD_DIR, 'settings.gradle'),
                  'w') as fout:
            fout.write('\n')
    ...
    def write_back(self):
        ...
        for idx, line in enumerate(self.original_lines):
            ...
            if m:
                ...
                if new_v != old_v:
                    self.target_lines[idx +
                                      1] = self.original_lines[idx +
                                                               1].replace(
                                                                   old_v,
                                                                   new_v)
        ...
        with open(os.path.join(self.project_root, self.BEAMMPLG_PATH),
                  'w') as fout:
